...
CLUSTER_NAME="From create_and_run_all.sh"
...
{"Args":["hive-script","--run-hive-script","--args","-f","'$QUERIES_DIR'/demo-010.q","-d","OUTPUT='$S3_BUCKET'/output/010"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"command-runner.jar","Properties":"","Name":"Demo 010"} ]' \
--name "$CLUSTER_NAME" --instance-groups '[{"InstanceCount":1,"InstanceGroupType":"CORE","InstanceType":"m3.xlarge","Name":"Core demo all"},{"InstanceCount":1,"InstanceGroupType":"MASTER","InstanceType":"m3.xlarge","Name":"Master demo all"}]' --scale-down-behavior TERMINATE_AT_TASK_COMPLETION $AUTOTERMINATE_OR_NOT --region $REGION
