...
--steps '[
  {"Args":["hive-script", "--run-hive-script", "--args", "-f","s3://reed9999/queries/data-engineering/hive/load-mini-gdelt-events.q", "-d","INPUT=s3://reed9999/events/_small", "-d","OUTPUT=s3://reed9999/output/step0.0","-hiveconf", "hive.support.sql11.reserved.keywords=false"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"command-runner.jar","Properties":"","Name":"step 0.0 load small subset of DB"},
